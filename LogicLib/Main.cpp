...
namespace LogicLib
{
	...
	ERROR_CODE Main::Init()
	{
		...
		m_pNetwork = std::make_unique<NetworkLib::TcpNetwork>();
	...
	{
		m_pServerConfig = std::make_unique<NetworkLib::ServerConfig>();
		...
		m_pLogger->Write(NetworkLib::LOG_TYPE::L_INFO, "%s | Port(%d), Backlog(%d)", __FUNCTION__, m_pServerConfig->Port, m_pServerConfig->BackLogCount);
		m_pLogger->Write(NetworkLib::LOG_TYPE::L_INFO, "%s | IsLoginCheck(%d)", __FUNCTION__, m_pServerConfig->IsLoginCheck);
		return ERROR_CODE::NONE;
	}
